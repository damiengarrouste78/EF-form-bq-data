...
-- ==> chargement via la webUI, avec la reconnaissance automatique de schema
...
select 
	case when lower(product_name) like '%organic%' then 1 else 0 end as flag_organic
	, count(*)
from instacart.products 
group by case when lower(product_name) like '%organic%' then 1 else 0 end
...
from 
	(
	select product_name, regexp_extract(lower(product_name), 'size ([0-9])') as size
	from instacart.products
	where aisle_id = 56 and lower(product_name) not like '%wipes%'
	) 
group by size
order by size
...
from 
	...
	select 
		...
		product_name, 
		nb_commandes, 
		rank() over(partition by department order by nb_commandes desc) as rk
	from
		(
		select 
		  	c.department, 
...
-- 2/ Créer une table au niveau client, avec le nb de commandes passées dans les départements 'frozen', 'bakery' et 'produce'
...
create table instacart.users_nb_commandes as 
select 
...
create table instacart.quantiles as
