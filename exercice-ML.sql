...
-- Le modèle à sélectionner est celui dont le nombre de clusters est contenu dans BEST_NUM_CLUSTERS
...
OPTIONS
  ( model_type='LOGISTIC_REG',
    ...
    DATA_SPLIT_EVAL_FRACTION = 0.25, -- Part dédié à l'évaluation
...
---------------- DEUXIEME MODELE ----------------
CREATE OR REPLACE MODEL instacart.MOD_TREE2
OPTIONS(MODEL_TYPE='BOOSTED_TREE_CLASSIFIER',
        DATA_SPLIT_METHOD='RANDOM', -- Méthode d'échantillonage
        DATA_SPLIT_EVAL_FRACTION = 0.25, -- Part dédié à l'évaluation
        NUM_PARALLEL_TREE = 1, -- Nombre d'arbres parallèles créés à chaque itération (valeur par défaut 1) 
...
-- Performances globales du modèle
SELECT *
FROM ML.EVALUATE (MODEL instacart.MOD_TREE2,
                 (SELECT * EXCEPT(user_id) FROM instacart.REGROUP_INFOS_M),
                 STRUCT(0.23))

-- Importance des variables
SELECT * from ML.FEATURE_IMPORTANCE(MODEL instacart.MOD_TREE2);

-- Matrice de confusion du modèle
SELECT *
FROM ML.CONFUSION_MATRIX (MODEL instacart.MOD_TREE2,
                          ( SELECT * FROM instacart.REGROUP_INFOS_M),
                          STRUCT(0.23))

-- Indicateurs liés à la courbe ROC du modèle                       
SELECT *
FROM ML.ROC_CURVE (MODEL instacart.MOD_TREE2,
                          ( SELECT * FROM instacart.REGROUP_INFOS_M))
